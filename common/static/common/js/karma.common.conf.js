...
var commonFiles = {
  ...
  sourceFiles: [{ pattern: "common/js/!(spec_helpers)/**/!(*spec).js" }],

  specFiles: [{ pattern: "common/js/spec_helpers/**/*.js" }],

  fixtureFiles: [{ pattern: "common/templates/**/*.underscore" }],
};
...
function coverageSettings(config) {
  ...
  return {
    ...
    reporters: [{ type: "cobertura", file: pth.file }, { type: "text-summary" }],
  };
}
...
function defaultNormalizeFunc(appRoot, pattern) {
  // eslint-disable-line no-shadow
  var pat = pattern;
  if (pat.match(/^common\/js/)) {
    pat = path.join(appRoot, "/common/static/" + pat);
  } else if (pat.match(/^xmodule_js\/common_static/)) {
    pat = path.join(appRoot, "/common/static/" + pat.replace(/^xmodule_js\/common_static\//, ""));
  }
  return pat;
}
...
function getBaseConfig(config, useRequireJs) {
  var getFrameworkFiles = function () {
    ...
    if (useRequireJs) {
      files = files.concat(["node_modules/requirejs/require.js", "node_modules/karma-requirejs/lib/adapter.js"]);
    }
...
function configure(config, options) {
  ...
  files.unshift({ pattern: path.join(appRoot, "common/static/common/js/jasmine.common.conf.js"), included: true });
  ...
  filesForCoverage = _.flatten(
    _.map(["sourceFiles", "specFiles"], function (collectionName) {
      return options[collectionName];
    })
  );
